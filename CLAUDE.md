...
Phase 0+1 (Gewicht) durchgestochen, Mahlzeiten-Domäne + Food-Memory live, Chat-Tools (lesen + schreiben für Gewicht, Mahlzeiten und persönliche Tagesziele) produktiv. Zwei Agenten teilen sich die Chat-UI (`apps/web/src/components/chat/`): Labor (`/api/chat`, alle Tools) und Coach (`/api/nutrition-coach`, gefiltertes Tool-Set + BMR/TDEE-Prompt). Foto-basierte Mahlzeit-Erkennung über `/api/recognize-meal` (Vision-LLM via Langdock), Barcode-Scan über `/api/lookup-barcode` (Open Food Facts). Trainings-Domäne durchgestochen: `workout_logged`-Event-Schema, Replay-Projektion, `logWorkout`/`retractWorkout` und ein minimaler `WorkoutLogSheet` mit Empty-State/Verlauf auf der Trainings-Seite leben — Chat-Tools und Workout-Templates folgen. Supabase `cjwgisdobzztljizrnfn` (eu-west-1), Demo-Login via `signInWithPassword`. Offen: echte `ai_interpretation`-Events.
