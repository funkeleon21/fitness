...
Eine Zeile pro Bereich — bewusst kompakt, damit parallele PRs konfliktfrei ergänzen können. Wenn ein PR mehrere Bereiche berührt, die jeweils zugehörige Zeile anfassen, keine Mono-Prosa.

- **Gewicht**: Phase 0+1 durchgestochen.
- **Mahlzeiten**: Domäne + Food-Memory live. Foto-Erkennung über `/api/recognize-meal` (Vision-LLM via Langdock; aktive Pantry-Items werden dem Modell als Match-Kontext mitgegeben, eindeutig zugeordnete Komponenten bekommen ihre kcal/Makros aus dem Pantry-Eintrag × `amount_g`/100 statt aus der LLM-Schätzung).
- **Vorrat (Pantry)**: `PantrySheet` mit Liste, Edit, Merge, Archiv (90-Tage-Lazy-Aging), Barcode-Eingang inkl. „bereits-vorhanden"-Hinweis und manuellem Fallback bei OFF-Miss, sowie Foto-Eingang für Nährwert-Tabellen (`/api/extract-nutrition-label`, Vision-LLM → Werte pro 100 g als Prefill im CreateSheet, User bestätigt vor Speichern). CRUD über `/api/pantry`. Barcode-Lookup `/api/lookup-barcode` cache-first über `pantry_items` + `pantry_barcodes`, Open Food Facts nur bei Miss. Nach Scan eines neuen Produkts zeigt sowohl der MealComposer als auch das PantrySheet einen Merge-Vorschlag bei ähnlichen Items.
- **Training**: Durchgestochen inkl. `WorkoutLogSheet` (mit Stimmungs-Smileys + freiem Notiz-Feld), `workout_templates`-Tabelle (mit wählbarem Icon pro Vorlage) und „Workout Memory"-Card.
- **Chat**: Zwei Agenten teilen sich die UI (`apps/web/src/components/chat/`): Labor (`/api/chat`, alle Tools — Gewicht, Mahlzeiten, Tagesziele, Training inkl. `log_workout_from_template`) und Coach (`/api/nutrition-coach`, gefiltertes Tool-Set + BMR/TDEE-Prompt — bewusst ohne Training-Tools).
- **Infra**: Supabase `cjwgisdobzztljizrnfn` (eu-west-1), Demo-Login via `signInWithPassword`.
- **Offen**: echte `ai_interpretation`-Events.
